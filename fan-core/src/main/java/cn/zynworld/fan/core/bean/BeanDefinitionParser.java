...
public class BeanDefinitionParser {
    ...
    private static void handleDefinitionDependent(BeanDefinition definition) {
        ...
        List<BeanDependent> fieldValueDependents = Arrays.stream(fields)
                .filter(field -> ObjectUtils.isNotNull(field.getAnnotation(PropertyValue.class)) && StringUtils.isNotEmpty(field.getAnnotation(PropertyValue.class).value()))
        ...
        List<BeanDependent> methodValueDependents = Arrays.stream(methods)
                .filter(method -> ObjectUtils.isNotNull(method.getAnnotation(PropertyValue.class)) && StringUtils.isNotEmpty(method.getAnnotation(PropertyValue.class).value()))
                .map(BeanDefinitionParser::handleResourceAnnotation)
                .collect(Collectors.toList());
    ...
    private static BeanDependent handleValueAnnotation(Field field) {
        ...
        dependent.setInjectInfo(propertyValue.value());
        return dependent;
    }
